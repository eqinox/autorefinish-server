...
module.exports = {
  async beforeCreate(event) {
    ...
    if (existingVideo) {
      throw new ValidationError(`Index ${index} already exists in the Video collection. Please choose another index.`);
    }
  },
  ...
  async beforeUpdate(event) {
    ...
    if (typeof index === 'undefined') {
      const entry = await strapi.db.query('api::image.image').findOne({
    ...
    if (existingVideo && existingVideo.id !== event.params.where.id) {
      throw new ValidationError(`Index ${index} already exists in the Video collection. Please choose another index.`);
    }
  },
